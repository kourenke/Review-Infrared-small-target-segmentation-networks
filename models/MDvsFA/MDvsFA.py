...
class G1G2Dataset(torch_data.Dataset):
    def __init__(self, mode):
        ...
        if self.mode == 'train':
            self.imageset_dir = os.path.join('./data/data/training/')
            self.imageset_gt_dir = os.path.join('./data/data/training/')
        elif self.mode == 'test':
            self.imageset_dir = os.path.join('./data/data/test_org/')
            self.imageset_gt_dir = os.path.join('./data/data/test_gt/')
        else:
...
if __name__ == '__main__':
    ...
    # 当前时间，日志文件的后缀
    time_suffix = time.strftime('%Y-%m-%d_%H_%M_%S',time.localtime(time.time()))
    # 日志文件
