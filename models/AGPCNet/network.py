...
class AGPCNet(nn.Module):
    def __init__(self, backbone='resnet18', scales=(10, 6), reduce_ratios=(8, 8), gca_type='patch', gca_att='origin',
        ...
        assert backbone in ['resnet18', 'resnet34'] #骨干网络的选择
        assert gca_type in ['patch', 'element'] #GCA类型
    ...
    def forward(self, x):
        ...
        out = self.context(c3)  #2 512 32 32

        out = F.interpolate(out, size=[hei // 4, wid // 4], mode='bilinear', align_corners=True) # 2 512 64 64
        out = self.fuse23(out, c2)

        out = F.interpolate(out, size=[hei // 2, wid // 2], mode='bilinear', align_corners=True)
        out = self.fuse12(out, c1)

        pred = self.head(out)    #2 1 128 128
        out = F.interpolate(pred, size=[hei, wid], mode='bilinear', align_corners=True)  #2 1 256 256
