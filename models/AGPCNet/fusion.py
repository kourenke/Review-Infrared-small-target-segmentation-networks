...
class AsymFusionModule(nn.Module):
    ...
    def forward(self, x_high, x_low):
        x_high = self.plus_conv(x_high)  #2 256 64 64
        pa = self.pa(x_low)              #2 256 64 64
        ca = self.ca(x_high)             #2 256 1 1

        feat = x_low + x_high            #2 256 64 64
        feat = self.end_conv(feat)       #2 256 64 64
        feat = feat * ca
        feat = feat * pa
        return feat