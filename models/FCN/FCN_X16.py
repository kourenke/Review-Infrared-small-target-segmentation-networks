...
class FCN_X16(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.conv_trans2 = nn.Conv2d(128, num_classes, 1)
        self.conv_trans3 = nn.Conv2d(256, 128, 1)

        self.upsample_8x = nn.ConvTranspose2d(num_classes, num_classes, 16, 8, 4, bias=False)
        self.upsample_8x.weight.data = bilinear_kernel(num_classes, num_classes, 16) #双线性插值初始化反卷积核
    ...
    def forward(self, x):
        ...
        add1 = self.upsample_2x_2(add1) #(1,256,64,64)
        add1 = self.conv_trans3(add1)  # (1,128,64,64)
        add2 = add1 + s2 #(1,128,64,64)

        output = self.conv_trans2(add2) #(1,1,64,64)
        output = self.upsample_4x(output) #(1,1,256,256)
        return output
