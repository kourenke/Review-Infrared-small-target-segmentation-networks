...
class FCN_X8(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.conv_trans2 = nn.Conv2d(64, num_classes, 1)
        self.conv_trans3 = nn.Conv2d(256, 128, 1)
        self.conv_trans4 = nn.Conv2d(128, 64, 1)

        self.upsample_8x = nn.ConvTranspose2d(num_classes, num_classes, 16, 8, 4, bias=False)
        self.upsample_8x.weight.data = bilinear_kernel(num_classes, num_classes, 16)

        self.upsample_4x = nn.ConvTranspose2d(num_classes, num_classes, 8, 4, 2, bias=False)
        self.upsample_4x.weight.data = bilinear_kernel(num_classes, num_classes, 8)

        self.upsample_2x_1 = nn.ConvTranspose2d(256, 256, 4, 2, 1, bias=False)
        self.upsample_2x_1.weight.data = bilinear_kernel(256, 256, 4)

        self.upsample_2x_2 = nn.ConvTranspose2d(128, 128, 4, 2, 1, bias=False)
        self.upsample_2x_2.weight.data = bilinear_kernel(128, 128, 4)

        self.upsample_2x_3 = nn.ConvTranspose2d(num_classes, num_classes, 4, 2, 1, bias=False)
        self.upsample_2x_3.weight.data = bilinear_kernel(num_classes, num_classes, 4)
    ...
    def forward(self, x):
        ...
        # s4 = self.upsample_2x_1(s4) #(1,512,32,32)
        # s4 = self.conv_trans1(s4) #(1,256,32,32)
        # add1 = s4 + s3 #(1,256,32,32)

        s3 = self.upsample_2x_1(s3) #(1,256,64,64)
        s3 = self.conv_trans3(s3) #(1,128,64,64)
        add1 = s3 + s2 #(1,128,64,64)

        # scores2 = self.scores2(add1) #通道数num_classes 12
        # add1 = self.conv_trans1(add1) #(1,256,16,16)
        add1 = self.upsample_2x_2(add1) #(1,128,128,128)
        add1 = self.conv_trans4(add1)  # (1,64,128,128)
        add2 = add1 + s1 # (1,64,128,128)

        output = self.conv_trans2(add2) #(1,1,128,128)
        output = self.upsample_2x_3(output) #(1,1,256,256)
        return output
