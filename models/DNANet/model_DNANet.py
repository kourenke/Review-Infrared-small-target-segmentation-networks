...
class DNANet(nn.Module):
    ...
    def forward(self, input):
        x0_0 = self.conv0_0(input)                                                     #1 16 256 256
        x1_0 = self.conv1_0(self.pool(x0_0))                                           #1 32 128 128
        x0_1 = self.conv0_1(torch.cat([x0_0, self.up(x1_0)], 1))                       #1 16 256 256

        x2_0 = self.conv2_0(self.pool(x1_0))                                           #1 64 64 64
        x1_1 = self.conv1_1(torch.cat([x1_0, self.up(x2_0),self.down(x0_1)], 1))       #1 32 128 128
        x0_2 = self.conv0_2(torch.cat([x0_0, x0_1, self.up(x1_1)], 1))                 #1 16 256 256

        x3_0 = self.conv3_0(self.pool(x2_0))                                           #1 128 32 32
        x2_1 = self.conv2_1(torch.cat([x2_0, self.up(x3_0),self.down(x1_1)], 1))       #1 64 64 64
        x1_2 = self.conv1_2(torch.cat([x1_0, x1_1, self.up(x2_1),self.down(x0_2)], 1)) #1 32 128 128
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, self.up(x1_2)], 1))           #1 16 256 256

        x4_0 = self.conv4_0(self.pool(x3_0))                                           #1 256 16 16
        x3_1 = self.conv3_1(torch.cat([x3_0, self.up(x4_0),self.down(x2_1)], 1))       #1 128 32 32
        x2_2 = self.conv2_2(torch.cat([x2_0, x2_1, self.up(x3_1),self.down(x1_2)], 1)) #1 64 64 64
        x1_3 = self.conv1_3(torch.cat([x1_0, x1_1, x1_2, self.up(x2_2),self.down(x0_3)], 1)) #1 32 128 128
        x0_4 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up(x1_3)], 1))           #1 16 256 256
        ...
        if self.deep_supervision:
            output1 = self.final1(x0_1)                   #1 16 256 256
            output2 = self.final2(x0_2)                   #1 16 256 256
            output3 = self.final3(x0_3)                   #1 16 256 256
            output4 = self.final4(Final_x0_4)             #1 16 256 256
...
if __name__ == "__main__":
    ...
    model =  DNANet(num_classes=1,input_channels=3, block=Res_CBAM_block, num_blocks=[2, 2, 2, 2], nb_filter=[16, 32, 64, 128, 256], deep_supervision=False)
    output = model(image)
    print(output.size())
